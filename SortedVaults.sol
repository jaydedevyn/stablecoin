...
contract SortedVaults {
    ...
    constructor(address _borrowingAddress) {
        data.maxSize = 10; 
    ...
    function _validInsertPosition(uint256 _NICR, address _prevId, address _nextId) internal view returns (bool) {
        if (_prevId == address(0) && _nextId == address(0)) {
            ...
            return data.head == _nextId && _NICR >= vaultManager.getNominalICR(_nextId);
        } else if (_nextId == address(0)) {
            // `(_prevId, null)` is a valid insert position if `_prevId` is the tail of the list
            return data.tail == _prevId && _NICR <= vaultManager.getNominalICR(_prevId);
        } else {
    ...
    modifier onlyBorrowingOrVaultManager {
        require(msg.sender == borrowingAddress || msg.sender == address(vaultManager),
